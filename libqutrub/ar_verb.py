...
## تستبدل الألف الممدودة في ,ل الكلمة بهمزة قطع بعدها همزة أخرى
	...
	len_word=len(word)
	while i<len_word and araby.isShortharaka(word[i]): # in HARAKAT:
		i+=1;
	word=word[i:]
	# convert all Hamza from into one form
...
#	print "len word",len(word);
	while i <len_word:
		if not araby.isShortharaka(word[i]):  # not in HARAKAT:
			word_nm+=word[i];
			if i+1 < len(word) and araby.isShortharaka(word[i+1]):# in HARAKAT:
				if word[i+1]==FATHA :
...
#						shakl+=ALEF;
					...
					if i+3>=len(word) or not araby.isShortharaka(word[i+3]):# not in HARAKAT:
						shakl+=WAW_HARAKA;
						i+=3;
					else :
						shakl+=DAMMA;
			...
			elif  i+1 < len(word) and araby.isHahaka(word[i+1]):# in HARAKAT :
				shakl+=word[i+1];
			else:
